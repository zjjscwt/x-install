...
setup_shortcut() {
  ...
  if ln -s "$SCRIPT_DIR/xray-manager.sh" "$target" 2>/dev/null; then
    echo "${GREEN}info:${RESET} 已创建快捷命令: daili"
  else
    if cp "$SCRIPT_DIR/xray-manager.sh" "$target" 2>/dev/null; then
      chmod +x "$target"
...
write_config() {
  ...
  sed -i \
    -e "s/你的UUID/$uuid/g" \
    -e "s/你的私钥/$private_key/g" \
    -e "s/www\.samsung\.com/$domain/g" \
    -e "s/\"shortIds\": \[\"a1b2c3d4\"\]/\"shortIds\": $short_ids_json/" \
    "$tmp_file"
